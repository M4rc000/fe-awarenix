...
export default function CardHeader({ reloadTrigger }: CardHeaderEmailTemplatesProps) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-4 w-full">
      {/* ─── Total Campaigns ─── */}
      <div className="flex flex-col rounded-xl border border-gray-200 bg-white p-4 min-h-[120px] dark:border-gray-800 dark:bg-white/[0.03] hover:shadow-md hover:-translate-y-2 transition duration-300 ease-in-out cursor-pointer">
        <div className="flex items-center justify-between mb-4">
          ...
          <h4 className="text-2xl font-bold text-gray-800 dark:text-white ml-4">
            {loading ? "..." : totalCampaigns.toLocaleString()}
          </h4>
        </div>
        <div className="flex justify-end mt-auto">
          {renderBadge(growthData)}
        </div>
      </div>
